...
instance_id=$(aws ec2 run-instances --image-id ami-8ac313ea --security-group-ids sg-890a37ed --count 1 --instance-type t2.large --key-name rstudio --instance-initiated-shutdown-behavior stop --query 'Instances[0].{d:InstanceId}' --output text --iam-instance-profile Name="Ben")
