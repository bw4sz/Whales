...
instance_id=$(aws ec2 run-instances --image-id ami-533ffb33 --security-group-ids sg-890a37ed --count 1 --instance-type r3.large --key-name rstudio --instance-initiated-shutdown-behavior stop --query 'Instances[0].{d:InstanceId}' --output text)
...
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i "/c/Users/Ben/.ssh/rstudio.pem" ubuntu@$dns "nohup bash ~/Job.bash" &
