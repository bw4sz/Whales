...
instance_id=$(aws ec2 run-instances --image-id ami-5fa96c3f --security-group-ids sg-890a37ed --count 1 --instance-type t2.medium --key-name rstudio --instance-initiated-shutdown-behavior terminate --query 'Instances[0].{d:InstanceId}' --output text)
...
nohup ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i "/c/Users/Ben/.ssh/rstudio.pem" ubuntu@$dns "bash ~/Job.bash" &
