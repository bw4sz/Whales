...
sxy<-split(mxy,mxy$Animal,mxy$Track,drop=T)
...
sxy<-split(traveling,traveling$Animal,traveling$Track,drop=T)
sxy<-lapply(sxy,function(x) {x$Index<-1:nrow(x); return(x)})
...
sxy<-split(foraging,foraging$Animal,foraging$Track,drop=T)
sxy<-lapply(sxy,function(x) {x$Index<-1:nrow(x); return(x)})
...
```{r}
ttime_unit<-data.frame(ccamlr$Unit,Hours=raster::extract(y=ccamlr,x=r,fun=sum,na.rm=T))
ggplot(nullhour,aes(x=ccamlr.Unit,y=value)) + geom_violin() + coord_flip() + geom_point(data=ttime_unit,col="red") + theme_bw()
```
...
ggplot(nullprop,aes(x=value)) + geom_density(fill="grey40") + facet_wrap(~Unit,scale="free",nrow=3) +theme_bw() + geom_vline(data=trueprop,aes(xintercept=value),col="black",size=1,linetype="dashed")
```
...
ggplot(nullp,aes(x=value)) + geom_density(aes(fill=Density)) + facet_wrap(~Unit,scale="free",nrow=2) +theme_bw() + geom_vline(data=trueprop,aes(xintercept=value),col="black",size=1,linetype="dashed") + scale_fill_continuous(low="blue",high="red")  + scale_x_continuous(labels = scales::percent) + labs(x="Proportion of time foraging")
