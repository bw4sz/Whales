...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=3000,n.burnin=2000,n.thin=4,DIC=FALSE))
}
...
ggmap(temp) + labs(fill="Mean Probabilty of Feeding")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9) + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red")+geom_polygon(data=frp,aes(x=long,y=lat,group=group),alpha=0.8,fill="snow2") 
...
#Traveling
travpar<-calc(rice,function(x){ inv.logit(1-(meanpar$`alpha_mu[1]` + meanpar$`beta2_mu[1]` * x))})
travdf<-data.frame(rasterToPoints(travpar))
...
```{r}
#Feeding
feedpar<-calc(distc,function(x){ inv.logit(1-(meanpar$`alpha_mu[2]` + meanpar$`beta2_mu[2]` * x))})
...
#Traveling
travpar<-calc(s,function(x,y){ inv.logit(1-(meanpar$`alpha_mu[1]` + meanpar$`beta_mu[1]` * x[1] + meanpar$`beta2_mu[1]` * x[2]))})
travdf<-data.frame(rasterToPoints(travpar))
...
#Feeding
feedpar<-calc(s,function(x){ inv.logit(1-(meanpar$`alpha_mu[2]` + meanpar$`beta_mu[2]` * x[1] + meanpar$`beta2_mu[2]` * x[2] ))})
feedf<-data.frame(rasterToPoints(feedpar))
