...
nullhour<-lapply(1:100,function(x) null_dist(camr,rnull))
...
nullhour<-merge(krill_unit,nullhour,by="Unit")

#just since 2009
nullhour<-nullhour %>% filter(Year %in%  "2009-2016")

#order by Krill catch unit
ord<-krill_unit %>% group_by(Unit) %>% filter(Year %in%  "2009-2016") %>% distinct(Density) %>% arrange(desc(Density)) %>% .$Unit 
#add in AP EAST
...
ggplot()  + geom_violin(data=nullhour,fill="black",aes(x=Unit,y=Days)) + coord_flip() + theme_bw() + labs(y="Hours") + labs(x="Unit",y="Observation Days") +geom_point(data=ttime_unit,col="grey50",size=3,aes(x=Unit,y=Days))
