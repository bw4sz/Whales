...
timed<-function(d,step_length){
  ...
  track_time<-d %>% group_by(Track) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% filter(mt>=48) %>% .$Track
...
x$step<-as.numeric(cut(as.POSIXct(x$timestamp),"9 hours"))
return(x)
})
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=2000,n.burnin=1900,n.thin=2,DIC=FALSE))
}
