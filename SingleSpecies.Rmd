...
#black and white
ggplot()+geom_path(data=mxy, aes(x=x, y=y,col=as.factor(Animal)),size=.5)  + facet_wrap(~Month,nrow = 2) + geom_path(data=fccamlr,aes(x=long,y=lat,group=Unit),size=0.3,col="black")  + borders(fill="grey10") + coord_cartesian(ylim = c(-68.9,-60),xlim=c(-51.396,-70)) + theme_bw() + mytheme + scale_fill_manual(values=rep("NA",8)) + scale_color_discrete(guide="none")
ggsave("Figures/MonthTracks.jpeg",height=6, width=9)
```
...
ggplot()+geom_path(data=mxy, aes(x=x, y=y,col=phistate,group=paste(Animal,Track)),size=.6) + geom_path(data=fccamlr,aes(x=long,y=lat,group=Unit),size=0.3,col="black")  + borders(fill="grey10") + coord_cartesian(ylim = c(-68.9,-60),xlim=c(-51.396,-70)) + theme_bw() + mytheme + scale_fill_manual(values=rep("NA",8)) + labs(col="Behavior")
ggsave("Figures/SpatialPred_BW.jpg",height=5,width=5)
