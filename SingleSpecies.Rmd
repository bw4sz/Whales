...
#grab a test animal
mxy[mxy$individual.local.identifier %in% c("112746","131142"),]
...
mxy$argos.lc<-factor(argos.lc,levels=c(3,2,1,0,"A","B"))
obs_class<-as.numeric(acast(mxy,Animal~Track~step~jStep,value.var="argos.lc"))
```
...
ggplot(mdf) + geom_point(aes(y=value,x=log(C1KRIcatchKG),col=Unit),size=3)  + theme_bw() + labs(y="Residency Time (Days)",x="Krill Catch (Log KG)") + labs(col="Behavior") + facet_wrap(~variable,nrow=1) + scale_color_brewer(palette="Accent")
ggsave("Figures/ResidencyTime_ccamlr.jpeg",height=3,width=8,dpi=300)

#correlation
krillbeh<-split(mdf,mdf$variable)
x<-krillbeh[[1]]

sapply(krillbeh,function(x){cor(x$value,log(x$C1KRIcatchKG),use="complete.obs")})
...
#BW version
ggplot() + geom_tile(data=data.frame(rasterToPoints(rforaging)),aes(x=x,y=y,fill=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=log(C1KRIcatchKG/1000))) + scale_fill_continuous(low="lightblue",high="red",limits=c(0,155)) + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) + ggtitle("Area-restricted Search") + labs(x="",y="") + borders(fill="grey90",alpha=0.4) + coord_cartesian(ylim = c(-69,-60),xlim=c(-53,-72)) + theme_bw() + mytheme
ggsave("Figures/ARSResidencyDays_Krill_BW.jpeg",height=4,width=6,dpi=300)

#BW version journal 
ggplot() + geom_tile(data=data.frame(rasterToPoints(rforaging)),aes(x=x,y=y,fill=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=log(C1KRIcatchKG/1000))) + scale_fill_continuous(low="grey95",high="black",limits=c(0,155)) + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) + ggtitle("Area-restricted Search") + labs(x="",y="") + borders(fill="grey90",alpha=0.4) + coord_cartesian(ylim = c(-69,-60),xlim=c(-53,-72)) + theme_bw() + mytheme
