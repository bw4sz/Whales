...
#grab set of animals
mxy<-mxy[mxy$Animal %in% c("121212","131111","112692","131116"," 131158"),]
...
timed<-function(d,step_length){
  ...
  track_time<-d %>% group_by(Track) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% filter(mt>=72) %>% .$Track
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=3000,n.burnin=2900,n.thin=1,DIC=FALSE))
}
