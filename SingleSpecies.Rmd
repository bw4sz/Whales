...
levels(fccamlr$Unit)<-c("Bransfield Strait East","Bransfield Strait West","Drake Passage East","Drake Passage West","AP East","Elephant Island","AP Pelagic Area","AP West")
...
ksp$Year<-cut(ksp$SeasonAbbreviation,c(1980,2009,2016),labels=c("1980-2009","2009-2016"))

#aggregate total by unit.
krill_unit<-as.data.frame(ksp) %>% group_by(Unit,Areakm2,Year) %>% summarize(Krill=sum(C1KRIcatchKG)/1000) %>% mutate(Density=Krill/Areakm2)
...
#BW version journal 
ggplot() + geom_tile(data=data.frame(rasterToPoints(rforaging)),aes(x=x,y=y,fill=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=C1KRIcatchKG/1000)) + scale_fill_continuous(low="grey95",high="black",limits=c(0,155)) + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) + ggtitle("Area-restricted Search") + labs(x="",y="") + borders(fill="grey90",alpha=0.4) + coord_cartesian(ylim = c(-69,-60),xlim=c(-53,-72)) + theme_bw() + mytheme
ggsave("Figures/ARSResidencyDays_Krill_BW_all.jpeg",height=4,width=6,dpi=300)
...
p<-ggplot() + geom_tile(data=data.frame(rasterToPoints(rtraveling)),aes(x=x,y=y,fill=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=C1KRIcatchKG/1000)) + scale_fill_continuous(low="lightblue",high="red",limits=c(0,155)) + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) + ggtitle("Traveling") + labs(x="",y="") + borders(fill="grey90",alpha=0.4) + coord_cartesian(ylim = c(-69,-60),xlim=c(-53,-72)) + theme_bw() + mytheme
p
ggsave("Figures/TravelingResidencyDays_Krill_BW.jpeg",height=4,width=6,dpi=300)

  ggplot() + geom_tile(data=data.frame(rasterToPoints(rtraveling)),aes(x=x,y=y,fill=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=C1KRIcatchKG/1000)) + scale_fill_continuous(low="grey95",high="black",limits=c(0,155)) + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) + ggtitle("Traveling") + labs(x="",y="") + borders(fill="grey90",alpha=0.4) + coord_cartesian(ylim = c(-69,-60),xlim=c(-53,-72)) + theme_bw() + mytheme
ggsave("Figures/TravelingResidencyDays_Krill_BW_all.jpeg",height=4,width=6,dpi=300)
...
null_dist<-function(){
  rnull<-rasterToPoints(r,sp=T)
  rnull$cell<-cellFromXY(r,rnull@coords)
  
  #take 0's layer
  rnull$layer<-sample(rnull$layer)
  
  #new raster object
  r_null<-raster(r)
  r_null[rnull$cell]<-rnull$layer
  
  time_unit<-raster::extract(y=ccamlr,x=r_null,fun=sum,na.rm=T)
  return(data.frame(ccamlr$Unit,Hours=time_unit))
}

nullhour<-lapply(1:100,function(x) null_dist())
nullhour<-melt(nullhour)
...
```{r}
ttime_unit<-data.frame(ccamlr$Unit,value=raster::extract(y=ccamlr,x=r,fun=sum,na.rm=T))
nullhour$ccamlr.Unit<-
levels(nullhour$ccamlr.Unit)<-c("Bransfield Strait East","Bransfield Strait West","Drake Passage East","Drake Passage West","AP East","Elephant Island","AP Pelagic Area","AP West")
levels(ttime_unit$ccamlr.Unit)<-c("Bransfield Strait East","Bransfield Strait West","Drake Passage East","Drake Passage West","AP East","Elephant Island","AP Pelagic Area","AP West")

ggplot(nullhour,aes(x=ccamlr.Unit,y=value)) + geom_violin() + coord_flip() + geom_point(data=ttime_unit,col="grey50",size=3) + theme_bw() + labs(y="Hours") + labs(x="Unit",y="Observation Days") + theme(plot.margin=margin(20,20,20,20))
ggsave("Figures/RandomizationI.jpeg",height=5,width=4)
```
...
nullR<-function(mxy){
  ...
  mxy$phi_null<-sample(c("Traveling","Area-restricted Search"),replace=T,size=nrow(mxy))
  #mxy$phi_null<-sample(mxy$phistate)
  
  nullt<-mxy %>% group_by(Unit,phi_null) %>% summarize(n=n())
  nullt[is.na(nullt$Unit),"Unit"]<-"Outside"
  nullt<-dcast(nullt,Unit~phi_null)
  
  #as fraction
  nullt$P_ARS<-nullt$`Area-restricted Search`/(nullt$Traveling+nullt$`Area-restricted Search`)
  return(nullt)
}

nullframe<-lapply(1:1000,function(x) nullR(mxy))
nullframe<-melt(nullframe)

#as a percentage and time seperately.
nulltimes<-nullframe %>% filter(variable %in% c("Traveling","Area-restricted Search"))

nullprop<-nullframe %>% filter(!variable %in% c("Traveling","Area-restricted Search"))

ggplot(nullprop,aes(x=value)) + geom_density(aes(fill=Unit)) + theme_bw()
```

Observed values

```{r}
truet<-mxy %>% group_by(Unit,phistate) %>% summarize(n=n())
truet[is.na(truet$Unit),"Unit"]<-"Outside"
truet<-dcast(truet,Unit~phistate)
...
levels(trueprop$Unit)<-c("Bransfield Strait East","Bransfield Strait West","Drake Passage East","Drake Passage West","Outside","AP Pelagic Area","AP West")

nullp$Unit<-as.factor(nullp$Unit)
levels(nullp$Unit)<-c("Bransfield Strait East","Bransfield Strait West","Drake Passage East","Drake Passage West","Outside","AP Pelagic Area","AP West")

ggplot(nullp,aes(x=value)) + geom_density(fill="grey60") + facet_wrap(~Unit,scale="free",nrow=2) +theme_bw() + geom_vline(data=trueprop,aes(xintercept=value),col="black",size=1,linetype="dashed") + scale_x_continuous(labels = scales::percent) + labs(x="Proportion of time foraging")
ggsave("Figures/RandomizationUnit.jpeg",height=4,width=9)
```

Add in krill catch.

```{r}
krill_unit[is.na(krill_unit$Unit),"Unit"]<-"Outside"

nullp<-merge(krill_unit,nullprop,by="Unit")

#just since 2009
nullp<-nullp %>% filter(Year %in%  "2009-2016")

#order by Krill catch unit
ord<-nullp %>% group_by(Unit) %>% distinct(Density) %>% arrange(desc(Density)) %>% .$Unit
nullp$Unit<-factor(nullp$Unit,levels=rev(ord))
ggplot(nullp,aes(x=Unit,y=value)) + geom_violin(aes(fill=Density)) +theme_bw() + geom_point(data=trueprop,col="grey50",size=3)  + scale_y_continuous(labels = scales::percent) + labs(y="Proportion of time foraging",fill="Krill Catch Desnity\n(Metric Tons/Area)") + coord_flip()
ggsave("Figures/RandomizationUnit.jpeg",height=4,width=9)
```
