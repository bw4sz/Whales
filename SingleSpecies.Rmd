...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=5000,n.burnin=4900,n.thin=2,DIC=FALSE))
}
...
mode_state<-pc %>% filter(pc$parameter %in% "state") %>% mutate(bstate=ifelse(value==1,"Traveling","Foraging")) %>% group_by(Animal,Track,step,bstate) %>% summarize(n=n()) %>% spread(key=bstate,value=n,fill=0) %>% mutate(ratio=Foraging/Traveling) %>% mutate(state=as.character(ifelse(ratio>=1,"Foraging","Traveling")))
...
ggmap(m)+geom_path(data=mxy, aes(x=x, y=y,col=bstate,group=paste(Animal,Track)),size=.2)   + theme_inset()+ geom_path(data=fccamlr,aes(x=long,y=lat,group=id),col="black",size=1) 
...
ggplot()+geom_path(data=mxy, aes(x=x, y=y,col=ratio,group=paste(Animal,Track)),size=.5) + facet_wrap(~individual.local.identifier,scales="free") + theme_inset() +scale_color_continuous(low='blue',high='red')
```

Compared to CMLRR regions

```{r}
ggmap(m)+ geom_polygon(data=fccamlr,aes(x=long,y=lat,group=id,alpha=0.5))+geom_path(data=mxy, aes(x=x, y=y,col=phi,group=paste(Animal,Track)),size=.5)   + scale_color_continuous(low='blue',high='red') + labs(col="Probability of Foraging")  + theme_inset() 
...
ggplot(data=mxy,aes(x=as.POSIXct(timestamp),y=bstate)) + geom_line(aes(group=Track),size=1) + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time",y="Behavior") + facet_wrap(~individual.local.identifier,ncol=3,scales="free")
```
...
names(s)<-c("All","Area Restricted Search","Traveling")
