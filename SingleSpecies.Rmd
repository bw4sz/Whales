...
mxy<-mxy[mxy$Animal %in% c("131134","131144"),]
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=100000,n.burnin=99000,n.thin=2,DIC=FALSE))
}
```
...
beta1prior<-data.frame(Draw=1:todraw,par='beta[1]',value=rnorm(todraw,-0.5,1.67),parameter='beta_mu',Behavior=1,Estimate="Prior")

beta2prior<-data.frame(Draw=1:todraw,par='beta[2]',value=rnorm(todraw,-0.5,1.67),parameter='beta_mu',Behavior=2,Estimate="Prior")

beta21prior<-data.frame(Draw=1:todraw,par='beta2[1]',value=rnorm(todraw,-0.5,1.67),parameter='beta2_mu',Behavior=1,Estimate="Prior")

beta22prior<-data.frame(Draw=1:todraw,par='beta2[2]',value=rnorm(todraw,-0.5,1.67),parameter='beta2_mu',Behavior=2,Estimate="Prior")
...
## Parameter Summary
```{r}
pc %>% filter(!parameter %in% c("phi","state")) %>% group_by(par) %>% summarize(mean=mean(value))
```
...
#mean env estimate
ggplot(postplot) + geom_ribbon(aes(x=x,y=mean,ymin=upper,ymax=lower,fill=State),alpha=0.5) + theme_bw() + labs(col="Transition",x="Distance to Coast (km)",y="Probability",fill="Transition") + facet_wrap(~Begin) + xlim(0,quantile(mxy$coast,0.99))
```
...
ggmap(temp) + labs(fill="Mean Probabilty of Feeding")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9) + theme_minimal() + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-70,-50) + ylim(-69,-59)
...
ggmap(temp) + labs(fill="Mean Probabilty of Feeding")+ geom_tile(data=feedf,aes(x=x, y=y,fill=layer),alpha=0.9) + theme_minimal() + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-70,-50) + ylim(-69,-59)
