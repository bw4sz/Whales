...
x$step<-as.numeric(cut(as.POSIXct(x$timestamp),"48 hours"))
return(x)
})
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=15000,n.burnin=10000,n.thin=5,DIC=FALSE))
}
...
#make phi a factor
mxy$phistate<-as.factor(mxy$phi>0.5)
levels(mxy$phistate)<-c("Traveling","Area-restricted Search")
```
