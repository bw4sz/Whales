...
ggplot(data=mxy,aes(x=as.POSIXct(timestamp),y=phi)) + geom_line(aes(group=Track),size=2.5) + labs("Probability of Foraging") + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time",y="Liklihood of Foraging") + facet_wrap(~individual.local.identifier,ncol=3,scales="free") + ylim(0,1)
```
...
#write to file
ggplot(as.data.frame(mksp)) + geom_point(aes(y=value,x=log(C1KRIcatchKG),col=variable))  + theme_bw() + labs(y="Residency Time (Days)",x="Krill Catch (Log Metric Tons)")
ggsave("Figures/Observed.svg")
```
...
levels(mxy$bstate)<-c("Traveling","Foraging")
...
ggsave("Figures/TravelingResidenceTime_JustTracks.jpeg",height=9,width=7)
```

```{r}
ggmap(temp) + geom_tile(data.frame(rasterToPoints(r)),aes(x=x,y=y,layer=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=C1KRIcatchKG/1000)) + scale_fill_continuous(low="blue",high="red") + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) 
```

```{r}
ggmap(temp) + geom_tile(data.frame(rasterToPoints(rforaging)),aes(x=x,y=y,layer=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=C1KRIcatchKG/1000)) + scale_fill_continuous(low="blue",high="red") + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) + ggtitle("Foraging") 
```

```{r}
ggmap(temp) + geom_tile(data.frame(rasterToPoints(rtraveling)),aes(x=x,y=y,layer=layer)) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=C1KRIcatchKG/1000)) + scale_fill_continuous(low="blue",high="red") + labs(fill="Days", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(1,5)) + ggtitle("Traveling")
```
