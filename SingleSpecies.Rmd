...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=2000,n.burnin=0,n.thin=8,DIC=FALSE))
}
...
gamma2prior<-data.frame(Draw=1:todraw,par='gamma[2]',value=rbeta(todraw,10,5),parameter='gamma',Behavior=2,Estimate="Prior")
...
spp<-lapply(sxy,function(x){
  ...
  m <- get_map(location=pp,zoom=7,source="google",scale=2,maptype="satellite",color="bw")
...
r<-raster(msp,ext=extent(c(-75,-50,-75,-50))*1.1)
res(r)<-0.1
...
#important geographic points
Areas<-data.frame(c("Gerlache Strait","Bransfield Strait"))
Lon<-c(-62.333,-62.7880)
Lat<-c(-64.5,-58.055)

iarea<-SpatialPointsDataFrame(cbind(Lon,Lat),data=Areas)  
plot(iarea,col='red',pch=13)
plot(ksp,add=T)
map(add=T)
