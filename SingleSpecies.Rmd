...
#grab two animals
mxy<-mxy[mxy$Animal %in% c("131132","131136"),]
...
timed<-function(d,step_length){
  ...
  #remove tracks that are shorter than two days
...
steps_all<-mxy %>% group_by(Animal,Track) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% mutate(n=floor(as.numeric(mt)/step_length))
