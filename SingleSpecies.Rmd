...
fccamlr<-fortify(ccamlr,region="S")
m <- get_map(location=pp,source="google",zoom=5,scale=2,maptype="satellite",color = "bw")
ggmap(m)+geom_path(data=mxy, aes(x=x, y=y,col=as.factor(Animal)),size=.5) + scale_color_discrete(guide="none") + geom_polygon(data=fccamlr,aes(x=long,y=lat,group=id,col=SS))
```
...
```{r}
m <- get_map(location=pp,source="google",zoom=5,scale=2,maptype="satellite",color="bw")
ggmap(m)+geom_path(data=mxy, aes(x=x, y=y,col=phi,group=paste(Animal,Track)),size=.5)   + scale_color_continuous(low='blue',high='red',limits=c(0,1)) + labs(col="Probability of Area Restricted Search State")  + theme_inset()
