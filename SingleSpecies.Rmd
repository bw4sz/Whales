...
```{r}
mdat<-read.csv("InputData/Antarctic Humpback overlap with krill fisheries -4891681668380779355.csv")
...
#inits
inl<-function(){
  list(gamma=list(0.729,0.02766),theta=list(0.0127,3.2779),beta_mu=list(-0.84124841,0.17825950),beta2_mu=list(0.04406852,-0.10138258),alpha=list(-0.22775441,-2.16712564))
}

#paramters to track
pt<-c("theta","gamma","phi","alpha_mu","beta_mu","beta2_mu")

if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",inits=inl,data=data,n.chains=2,parameters.to.save=pt,n.iter=20000,n.burnin=18000,n.thin=4,DIC=FALSE))
}
...
```{r,fig.width=11}
#Plot
ggplot(data=pc[!pc$parameter %in% c("phi"),],aes(x=value)) + geom_histogram() + facet_wrap(~par,scales='free',ncol=4) + theme_bw()  + ggtitle("Estimated parameters")
```
...
gamma1prior<-data.frame(Draw=1:todraw,par='gamma[1]',value=rbeta(todraw,6,2),parameter='gamma',Behavior=1,Estimate="Prior")

gamma2prior<-data.frame(Draw=1:todraw,par='gamma[2]',value=rbeta(todraw,2,6),parameter='gamma',Behavior=2,Estimate="Prior")
...
#mean env estimate
ggplot(postplot) + geom_ribbon(aes(x=x,y=mean,ymin=upper,ymax=lower,fill=State),alpha=0.5) + theme_bw() + labs(col="Transition",x="Ocean Depth (km)",y="Probability",fill="Transition") + facet_wrap(~Begin) + xlim(0,quantile(mxy$ocean,0.95))
```
...
#mean env estimate
ggplot(postplot) + geom_ribbon(aes(x=x,y=mean,ymin=upper,ymax=lower,fill=State),alpha=0.5) + theme_bw() + labs(col="Transition",x="Distance to Coast (km)",y="Probability",fill="Transition") + facet_wrap(~Begin) + xlim(0,quantile(mxy$coast,0.95))
```
