...
#mxy<-mxy[mxy$Animal %in% c("131151","131144"),]
...
```{r}
ggmap(m) + geom_point(data=mxy,aes(x=x,y=y,col=is.na(SST)),alpha=0.3)
ggmap(m) + geom_point(data=mxy,aes(x=x,y=y,col=is.na(ChloroA)),alpha=0.3)
ggmap(m) + geom_point(data=mxy,aes(x=x,y=y,col=is.na(OSU.Ocean.NPP.0.17deg.Monthly.NPP)),alpha=0.3)
```
...
timed<-function(d,step_length){
  ...
  track_time<-d %>% group_by(Track) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% filter(mt>=0) %>% .$Track
...
sxy<-lapply(sxy,timed,step_length=3)

#Format matrices for jags
mxy<-rbind_all(sxy)

######recode whales
mxy$OAnimal<-mxy$Animal
mxy$Animal<-as.numeric(as.factor(mxy$Animal))
...
x$Step<-as.numeric(cut(as.POSIXct(x$timestamp),"3 hours"))
return(x)
})
...
```{r,fig.height=15}
oxy$OAnimal<-oxy$Animal
ggplot() +  geom_path(data=oxy,col='blue',aes(x=x,y=y)) +  geom_path(data=mxy,col='red',aes(x=x,y=y),size=2,alpha=0.7)  + facet_wrap(~OAnimal,scales="free",ncol=4)
```
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=100000,n.burnin=99000,n.thin=3,DIC=FALSE))
}
...
#add priors
#alpha
a1prior<-data.frame(Draw=1:todraw,par='alpha[1]',value=rnorm(todraw,0.5,1.67),parameter='alpha_mu',Behavior=1,Estimate="Prior")

a2prior<-data.frame(Draw=1:todraw,par='alpha[2]',value=rnorm(todraw,0.5,1.67),parameter='alpha_mu',Behavior=2,Estimate="Prior")

beta1prior<-data.frame(Draw=1:todraw,par='beta[1]',value=rnorm(todraw,-1,1.67),parameter='beta_mu',Behavior=1,Estimate="Prior")

beta2prior<-data.frame(Draw=1:todraw,par='beta[2]',value=rnorm(todraw,-1,1.67),parameter='beta_mu',Behavior=2,Estimate="Prior")

beta21prior<-data.frame(Draw=1:todraw,par='beta2[1]',value=rnorm(todraw,-1,1.67),parameter='beta2_mu',Behavior=1,Estimate="Prior")

beta22prior<-data.frame(Draw=1:todraw,par='beta2[2]',value=rnorm(todraw,-1,1.67),parameter='beta2_mu',Behavior=2,Estimate="Prior")

gamma1prior<-data.frame(Draw=1:todraw,par='gamma[1]',value=rbeta(todraw,4,2),parameter='gamma',Behavior=1,Estimate="Prior")

gamma2prior<-data.frame(Draw=1:todraw,par='gamma[2]',value=rbeta(todraw,2,7),parameter='gamma',Behavior=2,Estimate="Prior")
...
#Traveling
travpar<-calc(rice,function(x){ inv.logit(meanpar$`alpha_mu[1]` + meanpar$`beta_mu[1]` * x)})
travdf<-data.frame(rasterToPoints(travpar))

ggmap(temp) + labs(fill="Mean Probabilty of Feeding")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9) + theme_minimal() + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-70,-50) + ylim(-67.5,-60)


#Feeding
feedpar<-calc(rice,function(x){ inv.logit(meanpar$`alpha_mu[2]` + meanpar$`beta_mu[2]` * x)})
feedf<-data.frame(rasterToPoints(feedpar))

ggmap(temp) + labs(fill="Mean Probabilty of Feeding")+ geom_tile(data=feedf,aes(x=x, y=y,fill=layer),alpha=0.9)  + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-70,-50) + ylim(-67.5,-60) + theme_inset()
```
...
#Traveling
travpar<-calc(distc,function(x){ inv.logit(meanpar$`alpha_mu[1]` + meanpar$`beta_mu[1]` * x)})
travdf<-data.frame(rasterToPoints(travpar))

ggmap(temp) + labs(fill="Mean Probabilty of Feeding")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9) + theme_minimal() + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-70,-50) + ylim(-67.5,-60)

#Feeding
feedpar<-calc(distc,function(x){ inv.logit(meanpar$`alpha_mu[2]` + meanpar$`beta_mu[2]` * x)})
feedf<-data.frame(rasterToPoints(feedpar))
...
#Traveling
travpar<-calc(s,function(x,y){ inv.logit(meanpar$`alpha_mu[1]` + meanpar$`beta_mu[1]` * x + meanpar$`beta2_mu[1]` * y)})
travdf<-data.frame(rasterToPoints(travpar))
...
ggmap(temp) + geom_tile(data=feedmap,aes(x=x, y=y,fill=layer),alpha=0.7) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,size=C1KRIcatchKG/1000)) + scale_fill_continuous(low="blue",high="red") + scale_color_continuous(low="white",high="black") + labs(fill="Probability of Feeding", size="Krill Catch (Metric Tons)") + scale_size_continuous(range=c(0,8)) + theme_inset()
```
