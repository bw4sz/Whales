...
#grab two animals
mxy<-mxy[mxy$Animal %in% c("131159","131153","131130"),]
...
steps_all<-mxy %>% group_by(Animal) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% mutate(n=floor(as.numeric(mt)/step_length))
...
idx<-as.matrix(table(mxy$Animal,mxy$Step))
...
for (x in 2:nrow(mxy)){
  mxy$j[x]<-as.numeric(as.POSIXct(mxy$timestamp[x])-as.POSIXct(mxy$timestamp[x-1]))/(step_length*60)
}
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=10000,n.burnin=9500,n.thin=2,DIC=FALSE))
}
...
##Spatial Prediction
```{r}
ggmap(m)+geom_path(data=mxy, aes(x=x, y=y,col=phi),size=.5)+ scale_color_continuous(low='blue',high='red') + labs(col="Probability of Feeding") 
```

```{r,fig.height=15,fig.width=12}
sxy<-split(mxy,mxy$Animal)
spp<-lapply(sxy,function(x){
  pp<-c(mean(x$x),mean(x$y))
  m <- get_map(location=pp,source="google",zoom=7,scale=2,maptype="satellite")
  ggmap(m)+geom_path(data=x, aes(x=x, y=y,col=phi),size=.5)   + scale_color_continuous(low='blue',high='red') + labs(col="Probability of Feeding") + facet_wrap(~Animal) + theme_bw()
})
spp
```
...
ggplot(data=mxy,aes(x=as.POSIXct(timestamp),y=phi,col=ocean)) + geom_line(aes(group=Animal),size=2.5) + labs("Probability of Feeding") + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time (hour)",y="Liklihood of Feeding") + facet_wrap(~Animal,ncol=2,scales="free") + ylim(0,1)
```
