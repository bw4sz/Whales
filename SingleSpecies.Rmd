...
timed<-function(d,step_length){
  ...
  track_time<-d %>% group_by(Track) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% filter(mt>=20) %>% .$Track
...
#paramters to track
pt<-c("theta","gamma","phi","alpha_mu","beta_mu","beta2_mu","beta","beta2","state")

if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=60000,n.burnin=55000,n.thin=10,DIC=FALSE))
}
...
#update function
recompile(jagM)
jagM <- update(jagM,n.iter=20000,n.burnin=17000,n.thin=10) 
```
...
#mean env estimate
ggplot(allplot[allplot$Begin=="Traveling" & allplot$State == "Foraging",]) + geom_line(aes(x=x,y=mean,ymin=upper,ymax=lower,col=Month),alpha=0.5,size=1) + theme_bw() + labs(col="Month",x="Ocean Depth (km)",y="Probability of Switching",fill="Month") 
```
...
```{r}
ggplot(allplot[allplot$Begin=="Traveling" & allplot$State == "Foraging",]) + geom_ribbon(aes(x=coast,y=mean,ymin=upper,ymax=lower,fill=Month),alpha=0.5,size=1) + theme_bw() + labs(col="Transition",x="Distance to coast (km)",y="Probability of Feeding",fill="Month") + xlim(0,quantile(mxy$coast,0.5))

#mean env estimate
ggplot(allplot[allplot$Begin=="Traveling" & allplot$State == "Foraging",]) + geom_ribbon(aes(x=coast,y=mean,ymin=upper,ymax=lower,fill=Month),alpha=0.5,size=1) + theme_bw() + labs(col="Transition",x="Distance to coast (km)",y="Probability of Switching",fill="Month") 
```
...
```{r,eval=F}
a<-pc %>% filter(!Behavior==1|is.na(Behavior)) %>% filter(parameter %in% c("phi","state")) 
a<-dcast(a,Draw+chain+Animal+Track+step~parameter)
ggplot(a,aes(x=phi,y=state-1)) + geom_point() + geom_smooth(method="glm",method.args=list(family="binomial"))

ggplot(a) + geom_density(aes(x=1-phi,fill=as.factor(state)),alpha=0.5) + theme_bw() + labs(fill="Current Behavior",x="Probability of Switching to Foraging") + scale_fill_manual(labels=c("Traveling","Foraging"),values=c("blue","red"))
...
```{r,fig.height=20}
#phi and ocean plot
ggplot(data=mxy,aes(x=ocean,y=logit(phi),col=Month)) + geom_point() + labs(x="Ocean Depth",y="Estimated Log Odds of Foraging") + facet_wrap(~individual.local.identifier,ncol=3,scales="free") + geom_smooth(method="lm")
```
...
```{r,fig.height=20}
#phi and ocean plot
ggplot(data=mxy,aes(x=coast,y=logit(phi),col=Month)) + geom_point() + labs(x="Distance to coast",y="Estimated Log Odds of Foraging") + facet_wrap(~individual.local.identifier,ncol=3,scales="free")+ geom_smooth(method="lm")
```
...
ggplot(data=mxy,aes(x=as.POSIXct(timestamp),y=phi,col=ocean)) + geom_line(aes(group=Track),size=2.5) + labs("Probability of Foraging") + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time",y="Liklihood of Foraging") + facet_wrap(~Animal,ncol=3,scales="free") + ylim(0,1)
```
...
ggmap(temp) + labs(fill="Mean Probabilty of Foraging")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9) + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red",limits=c(0,1)) + xlim(-75,-50) + ylim(-70,-55) 
```
...
ggmap(temp) + labs(fill="Mean Probabilty of Foraging")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9) + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-75,-50) + ylim(-70,-55)
```
...
ggmap(temp) + labs(fill="Mean Probabilty of Foraging")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9)  + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red")+theme(axis.text.x=element_blank(),axis.text.y=element_blank()) + xlim(-75,-50) + ylim(-70,-55) + labs(x="",y="")
```
