...
#paramters to track
pt<-c("theta","gamma","gamma_mu","phi","alpha_mu","state")
...
#single index
splitpc[c("alpha_mu","gamma_mu","theta")]<-lapply(splitpc[c("alpha_mu","gamma_mu","theta")],function(x){
...
#double index
#single index
splitpc[c("gamma")]<-lapply(splitpc[c("gamma")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+),(\\d+)]"))[,3:4]
    colnames(sv)<-c("Behavior","Animal")
    pc<-data.frame(x,sv)
    return(pc)
})
...
ggplot(pc[!pc$parameter %in% c("phi","state","gamma"),],aes(x=Draw,y=value)) + facet_wrap(~par,scales="free",ncol=3) + geom_line(aes(col=as.factor(chain))) + labs(col="Chain")
```
...
```{r,fig.height=10}
#Plot
ggplot(data=pc[!pc$parameter %in% c("phi","state","gamma"),],aes(x=value)) + geom_histogram() + facet_wrap(~par,scales='free',ncol=3) + theme_bw()  + ggtitle("Estimated parameters")
```

### Variation in autocorrelation by individual
```{r,fig.height=10}
ggplot(data=pc[pc$parameter %in% c("gamma"),],aes(x=Animal,y=value,fill=Behavior)) + geom_violin()  + theme_bw()  + ggtitle("Estimated parameters") + coord_flip()
```
...
gamma1prior<-data.frame(Draw=1:todraw,par='gamma_mu[1]',value=rbeta(todraw,5,2),parameter='gamma_mu',Behavior=2,Estimate="Prior")

gamma2prior<-data.frame(Draw=1:todraw,par='gamma_mu[2]',value=rbeta(todraw,5,2) * runif(todraw,0,0.75),parameter='gamma_mu',Behavior=1,Estimate="Prior")
...
psum<-pc %>% filter(!parameter %in% c("phi","state","gamma")) %>% group_by(parameter,par) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
