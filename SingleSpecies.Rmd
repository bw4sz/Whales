...
mdat$coast<-mdat$NASA.Distance.to.Coast
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=20,n.burnin=0,n.thin=1,DIC=FALSE))
}
...
postplot<-pc %>% filter(parameter %in% c('beta_mu','alpha_mu')) %>% dcast(.,Draw+chain~par) %>% group_by(Draw,chain) %>% do(dplot(a1=c(.$'alpha_mu[1]',.$'alpha_mu[2]'),beta=c(.$'beta_mu[1]',.$'beta_mu[2]'),x=mxy$ocean)) %>% group_by(x=x,Begin,State) %>% summarize(mean=mean(y),lower=quantile(y,0.05),upper=quantile(y,0.95))
...
postplot<-pc %>% filter(parameter %in% c('beta2_mu','alpha_mu')) %>% dcast(.,Draw+chain~par) %>% group_by(Draw,chain) %>% do(dplot(a1=c(.$'alpha_mu[1]',.$'alpha_mu[2]'),beta=c(.$'beta2_mu[1]',.$'beta2_mu[2]'),x=mxy$coast)) %>% group_by(x=x,Begin,State) %>% summarize(mean=mean(y),lower=quantile(y,0.05),upper=quantile(y,0.95))
...
ggplot() + geom_tile(data=feedmap,aes(x=x, y=y,fill=layer),alpha=0.8) + geom_point(data=krill,aes(x=GridMidpointDegreeLon,y=GridMidpointHalfDegreeLat,col=log(C1KRIcatchKG))) + theme_minimal() + scale_fill_continuous(low="blue",high="red") + scale_color_continuous(low="white",high="black") + ylim(-70,-60) + xlim(-50,-70) + labs(col="Krill Catch (log(KG))",fill="Probability of Feeding") 
```
