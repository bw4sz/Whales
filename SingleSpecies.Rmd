...
mdat$ocean<-mdat$NASA.Distance.to.Coast
...
#grab two animals
#mxy<-mxy[mxy$Animal %in% c("131132","112737","131156","12326","123232","112705","154187","121210","131134","131141","112701","131159"),]
mxy<-mxy[mxy$Animal %in% c("112703","121211"),]
...
mxy<-mxy[!mxy$NASA.Distance.to.Coast<0,]
```
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=1500,n.burnin=0,n.thin=3,DIC=FALSE))
}
