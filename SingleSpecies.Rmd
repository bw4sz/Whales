...
nullR<-function(mxy){
  ...
  nullt<-mxy %>% group_by(Unit,phi_null) %>% summarize(n=n()) %>% filter(!is.na(Unit))
...
```{r}
truet<-mxy %>% group_by(Unit,phistate) %>% summarize(n=n()) %>% filter(!is.na(Unit))
...
ggplot(nullrange,aes(x=Unit)) + geom_point(data=trueprop,aes(y=value),col="black",size=4)  + geom_linerange(col="grey60",aes(x=Unit,ymin=lower,ymax=upper),size=2)+ scale_y_continuous(labels = scales::percent) +labs(y="Proportion of time foraging") + theme_bw() + coord_flip()
ggsave("Figures/RandomizationI.jpeg",height=4,width=5)
```
