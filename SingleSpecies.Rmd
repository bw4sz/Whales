...
#paramters to track
pt<-c("theta","gamma","phi","alpha_mu","beta_mu","beta2_mu","beta3_mu","state")

if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=6000,n.burnin=4000,n.thin=8,DIC=FALSE))
}

```

##Chains
```{r,fig.height=10,eval=T}
#bind chains
...
#single index
splitpc[c("alpha_mu","beta_mu","beta2_mu","beta3_mu","gamma","theta")]<-lapply(splitpc[c("alpha_mu","beta_mu","beta2_mu","beta3_mu","gamma","theta")],function(x){
...
```{r}
  ggmap(m)+geom_path(data=mxy, aes(x=x, y=y,col=phi),size=.5)   + scale_color_continuous(low='blue',high='red',limits=c(0,1)) + labs(col="Probability of Foraging")  + theme_inset()
```
...
spp<-lapply(sxy,function(x){
  ...
  m <- get_map(location=pp,source="google",scale=2,maptype="satellite",color="bw")
...
```{r,fig.height=12}
#phi and ocean plot
ggplot(data=mxy,aes(x=ocean,y=logit(phi))) + geom_point() + labs(x="Ocean Depth",y="Estimated Log Odds of Foraging") + facet_wrap(~Animal,ncol=4,scales="free")
```
...
```{r,fig.height=10}
#phi and ocean plot
ggplot(data=mxy,aes(x=coast,y=logit(phi))) + geom_point() + labs(x="Distance to coast",y="Estimated Log Odds of Foraging") + facet_wrap(~Animal,ncol=4,scales="free")
```
...
allx$beta3<-filter(pc,par %in% 'beta3_mu[1]') %>% group_by(parameter) %>% summarize(m=mean(value)) %>% .$m

traj<-function(ocean,coast,alpha,beta1,beta2,beta3){
  p<-1-inv.logit(alpha + beta1*ocean + beta2*coast + beta3*coast * ocean)
  data.frame(ocean,coast,phi=p)
}

intplot<-allx %>% group_by(ID) %>% do(traj(.$ocean,.$coast,.$alpha,.$beta1,.$beta2,.$beta3))
...
ggmap(temp) + labs(fill="Mean Probabilty of Foraging")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9) + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-75,-50) + ylim(-70,-55) +  geom_tile(data=data.frame(rasterToPoints(mice[[14]])),aes(x=x,y=y,fill=Feb_2013),alpha=0.8,fill='snow2') + ggtitle("Feb 2013") 
```
...
```{r}
meanpar<-pc %>% filter(parameter %in% c('beta_mu',"beta2_mu","beta3_mu",'alpha_mu'))  %>% group_by(par) %>% summarize(mean=mean(value)) %>% dcast(...~par)

##Bathymetry
s<-stack(rice,distc)

#Traveling
travpar<-calc(s,function(x,y){ 1-inv.logit(meanpar$`alpha_mu[1]` + meanpar$`beta_mu[1]` * x[1] + meanpar$`beta2_mu[1]` * x[2] + meanpar$`beta3_mu[1]` * x[2]*x[1])})
...
pall1[[mn]]<-ggmap(temp) + labs(fill="Mean Probabilty of Foraging")+ geom_tile(data=travdf,aes(x=x, y=y,fill=layer),alpha=0.9)  + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red")+ geom_tile(data=data.frame(rasterToPoints(mice[[months[mn]]])),aes(x=x,y=y,fill=names(mice)[[months[mn]]]),alpha=0.8,fill='snow2') + ggtitle(names(mice)[[months[mn]]])+theme(axis.text.x=element_blank(),axis.text.y=element_blank()) + xlim(-75,-50) + ylim(-70,-55) + labs(x="",y="")
}
pall1
```
...
#toname<-kdf %>% filter(ksuit>0.9)
#toname<-SpatialPointsDataFrame(cbind(toname$GridMidpointDegreeLon,toname$GridMidpointHalfDegreeLat),toname)
#toname
