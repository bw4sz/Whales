...
```{r}
  cl<-rainbow(100)
  cl<-sample(cl)
  p<-ggplot()+geom_path(data=mxy, aes(x=x, y=y,col=as.numeric(step),group=paste(Track,step)),size=1) + labs(x="",y="") + theme(axis.text.x=element_blank(),axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.ticks.x=element_blank()) + scale_color_gradientn(colours = cl) + theme_bw() + facet_wrap(~Animal,scales="free") + mytheme
```
...
ggplot()+geom_path(data=mxy, aes(x=x, y=y,col=ratio,group=paste(Animal,Track)),size=.5) + facet_wrap(~individual.local.identifier,scales="free") + mytheme +scale_color_continuous(low='blue',high='red')
```
