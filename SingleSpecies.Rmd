...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=4000,n.burnin=3000,n.thin=2,DIC=FALSE))
}
...
spp<-lapply(sxy,function(x){
  ...
  m <- get_map(location=pp,zoom=4,source="google",scale=1,maptype="satellite")
