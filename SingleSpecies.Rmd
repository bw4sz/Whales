...
stepmxy<-split(mxy,list(mxy$Step,mxy$Track,mxy$Animal),drop = TRUE)
...
spp<-lapply(sxy,function(x){
  pp<-c(mean(x$x),mean(x$y))
  m <- get_map(location=pp,source="google",zoom=7,scale=2,maptype="satellite")
  ggmap(m)+geom_path(data=x, aes(x=x, y=y,col=phi),size=.5)   + scale_color_continuous(low='blue',high='red') + labs(col="Probability of Feeding") + facet_wrap(~Track) + theme_bw()
})
spp
```
