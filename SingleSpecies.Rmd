...
p<-coda.samples(model=jags,n.iter=20000,thin=10,variable.names=c("theta","gamma","alpha","beta"))
```
...
behav<-coda.samples(model=jags,n.iter=2000,n.thin=5,variable.names=c("state"))
...
#As matrices
sv<-data.frame(str_match(behav_chains$par,"(\\w+)\\[(\\d+)]"))[,-1]
colnames(sv)<-c("parname","index")

#format to combine
behav_chains<-data.frame(behav_chains,sv)
colnames(behav_chains)[3]<-"Behavior"
colnames(behav_chains)[5]<-"step"

#combine with spatial prediction
spp_b<-merge(toplot,behav_chains,by=c("Draw","step"))

#arrange by step
spp_b$step<-as.numeric(as.character(spp_b$step))
spp_b<-spp_b %>% arrange(step)

#Label Behaviors
spp_b$Behavior<-as.factor(spp_b$Behavior)
levels(spp_b$Behavior)<-c("Traveling","Feeding")

ggplot(data=spp_b,aes(x=x,y=y,col=Behavior)) + geom_point() + theme_bw() + geom_path(data=mdat_df,aes(x=location.long,y=location.lat),col='red')

#Consensus behavior
tb<-as.data.frame(table(spp_b$step,spp_b$Behavior))
tb<-dcast(tb,Var1~Var2)
colnames(tb)[1]<-c("step")
tb$LFeeding<-tb$Feeding/(tb$Traveling + tb$Feeding)

#merge with input
mdat_df$step<-1:nrow(mdat_df)
mdat_df<-merge(mdat_df,tb,by="step")

m <- get_map(location=pp,source="google",zoom=6,scale=2,maptype="terrain")
ggmap(m)+geom_path(data=mdat_df, aes(x=location.long, y=location.lat,col=LFeeding),size=2) + scale_color_continuous(low='blue',high='red') + labs(col="Probability of Feeding State")
```

##Autocorrelation in behavior

```{r}
ggplot(data=mdat_df,aes(x=as.numeric(step),y=LFeeding,col=OceanDepth)) + geom_line(aes(group=1),size=2.5) + labs("Liklihood Feeding") + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time Step",y="Liklihood of Feeding")
```
