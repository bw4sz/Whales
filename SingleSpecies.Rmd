...
#mxy<-mxy[mxy$Animal %in% c("121211","123224","154186"),]
...
timed<-function(d,step_length){
  ...
  track_time<-d %>% group_by(Track) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% filter(mt>=4) %>% .$Track
...
ggmap(temp) + labs(fill="Mean Probabilty of Feeding")+ geom_tile(data=feedf,aes(x=x, y=y,fill=layer),alpha=0.9)  + scale_color_discrete(guide="none") + scale_fill_continuous(low="blue",high="red") + xlim(-70,-50) + ylim(-67.5,-60)
```
...
#Feeding
feedpar<-calc(s,function(x){ inv.logit(meanpar$`alpha_mu[2]` + meanpar$`beta_mu[2]` * x[1] + meanpar$`beta2_mu[1]` * x[2] )})
feedf<-data.frame(rasterToPoints(feedpar))
