...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=12000,n.burnin=11000,n.thin=4,DIC=FALSE))
}
...
postplot<-pc %>% filter(parameter %in% c('lbeta_mu','lalpha_mu')) %>% dcast(.,Draw+chain~par) %>% group_by(Draw,chain) %>% do(dplot(a1=c(.$'lalpha_mu[1]',.$'lalpha_mu[2]'),beta=c(.$'lbeta_mu[1]',.$'lbeta_mu[2]'),x=mxy$ocean)) %>% group_by(x=x,Draw,chain,Begin,State) %>% summarize(mean=mean(y),lower=quantile(y,0.05),upper=quantile(y,0.95))
...
spp<-lapply(sxy,function(x){
  ...
  m <- get_map(location=pp,zoom=5,source="google",scale=1,maptype="satellite")
