...
ggplot(data=spp_b,aes(x=location.long,y=location.lat,col=Behavior)) + geom_point() + theme_bw() + geom_path(data=mdat_df,aes(x=x,y=y),col='red')

#merge with input
mdat_df$step<-1:nrow(mdat_df)
mdat_df<-merge(mdat_df,tb,by="step")

ggplot()+geom_path(data=mdat_df, aes(x=location.long, y=location.lat,col=Probability),size=2) + scale_color_continuous(low='blue',high='red') + labs(col="Probability of Feeding")
```

##Autocorrelation in behavior

```{r}
ggplot(data=mdat_df,aes(x=as.numeric(step),y=Probability,col=ocean)) + geom_line(aes(group=1),size=2.5) + labs("Probability of Feeding") + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time Step",y="Liklihood of Feeding")
```
