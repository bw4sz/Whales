...
mxy<-mxy[mxy$Animal %in% c("131144","131159","123224"),]
...
#inits
inl<-function(){
  list(gamma=c(0.729,0.02766),beta_mu=c(-0.84124841,0.17825950),beta2_mu=c(0.04406852,-0.10138258),alpha_mu=c(-0.2277,-2.1671))
}
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",inits=inl,data=data,n.chains=2,parameters.to.save=pt,n.iter=30000,n.burnin=29000,n.thin=4,DIC=FALSE))
}
```
...
feeddf<-data.frame(rasterToPoints(feedpar))
