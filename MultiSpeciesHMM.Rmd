...
```{r}
traj<-function(gamma=gamma,theta=theta,habitat=habitat,a1_mu,a1_sd,beta_mu,beta_sd,steps=steps){
...
step.ind<-rpois(ind,100)

if(newModel){
    for(x in 1:ind){
      mxy[[x]]<-traj(gamma=gamma,theta=theta,habitat=habitat,a1_mu=a1_mu,a1_sd=a1_sd,beta_mu=beta_mu,beta_sd=beta_sd,steps=step.ind[x])
  } 
...
if(newModel){
  jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=10000,n.burnin=8000,n.thin=4,DIC=FALSE)
}
...
```{r}
ggplot(data=mxy,aes(x=as.numeric(Step),y=phi,col=ocean)) + geom_line(aes(group=Animal),size=2.5) + labs("Probability of Feeding") + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time Step",y="Liklihood of Feeding") + facet_wrap(~Animal)
```
...
state_shade<-lapply(sxy,function(x){
  ...
  state_shade<-data.frame(xmin=c(0,cumsum(state_change$lengths[-length(state_change$lengths)])),xmax=cumsum(state_change$lengths),Behavior=state_change$values)
})
...
ggplot() + geom_rect(data=state_shade,aes(xmin=xmin,xmax=xmax,fill=Behavior), ymin=-Inf,ymax=Inf) + facet_wrap(~Animal,ncol=1) + theme_bw() + geom_line(data=mxy,aes(x=Step,y=phi)) + ylim(0,1) + labs(fill="True Behavior",y="Estimated Probability of Feeding")
```
...
cox_frame<-data.frame()
for(x in 1:nrow(runs)){
  
}
runs[runs==Beha]

cox_intercept<-coxph(Surv(as.numeric(x$step),as.numeric(x$value))~1,data=x)
Sim1 <- coxsimLinear(cox_intercept,qi="Relative Hazard",Xj = c(0,200))
# Plot
simGG(Sim1)
