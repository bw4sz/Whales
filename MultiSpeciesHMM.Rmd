...
if(newModel){
  habitat<-raster(xmn=-100,xmx=100,ymn=-100,ymx=100,resolution=0.25,crs=NA,vals=runif(640000,0,0.45))
}
...
ggplot(mxy,aes(x=x,y=y)) + theme_bw() + coord_cartesian(xlim=bb[1,],ylim=bb[2,]) +  geom_raster(data=habitatdf,aes(x=x,y=y,fill=layer)) + scale_fill_continuous("Ocean Depth",low='grey90',high='black')    + geom_path(aes(col=State,group=Animal),size=1) 
...
system.time(update(jags,2000))
p<-coda.samples(model=jags,n.iter=100,thin=2,variable.names=c("theta","gamma","alpha","beta","phi"))
```
...
splitpc[c("alpha","beta","gamma","theta")]<-lapply(splitpc[c("alpha","beta","gamma","theta")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,-2]
    colnames(sv)<-c("parname","Behavior")
    pc<-data.frame(x,sv)
})
...
#Triple Index 
splitpc[c("phi")]<-lapply(splitpc[c("d","phi")],function(x){
#As matrices
...
colnames(pc)<-c("Draw","par","value","chain","paramater","parname","Animal","step","Behavior")

#plot all but d
ggplot(pc[!pc$parameter %in% c("d","phi"),],aes())
```
...
#add priors
#alpha
a1prior<-data.frame(Draw=1:todraw,par='alpha[1]',value=inv.logit(rnorm(todraw,0,1.6)),parname='alpha',index=1,Estimate="Prior")

a2prior<-data.frame(Draw=1:todraw,par='alpha[2]',value=inv.logit(rnorm(todraw,0,1.6)),parname='alpha',index=2,Estimate="Prior")

beta1prior<-data.frame(Draw=1:todraw,par='beta[1]',value=inv.logit(rnorm(todraw,0,1.6)),parname='beta',index=1,Estimate="Prior")

beta2prior<-data.frame(Draw=1:todraw,par='beta[2]',value=inv.logit(rnorm(todraw,0,1.6)),parname='beta',index=2,Estimate="Prior")
