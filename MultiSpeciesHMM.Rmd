...
traj<-function(gamma=gamma,theta=theta,habitat=habitat,a1_mu,a1_sd,beta_mu,beta_sd,total_time=total_time,step_length){
  ...
  timestep<-sort(runif(100,0,total_time) * 24)  
...
if(newModel){
  ...
  mxy<-melt(mxy,id.vars=c("x","y","Step","Hour","State","ocean"))
  #Label Individual
...
#time diff array
j<-acast(mxy,Animal~Step,value.var="Stepi")

data=list(argos=obs,steps=steps,R=R,ocean=oc,ind=ind,j=j)

#paramters to track
pt<-c("theta","gamma","phi","lalpha_mu","lalpha_tau","lbeta_mu","lbeta_tau","state")

if(newModel){
  jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=1000,n.burnin=800,n.thin=4,DIC=FALSE)
}
