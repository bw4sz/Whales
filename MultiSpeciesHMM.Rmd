...
####Process Model
  ...
  for (x in 2:(steps-1)){
    ...
    #Behavior
    phi[x,1] <- inv.logit(a1+beta*ocean[x])
    phi[x,2]<- 1 - phi[x,1]
...
dplot<-function(a1,beta,x){
  ...
  d11<-data.frame(x,y,State="Traveling")
  
  y<-1-inv.logit(a1+beta*x)
  d12<-data.frame(x,y,State="Feeding")
  
  d<-rbind_all(list(d11,d12))
}
```
...
if(newModel){
  habitat<-raster(xmn=-100,xmx=100,ymn=-100,ymx=100,resolution=0.25,crs=NA,vals=runif(640000,0,4))
}

#Visualize
d<-dplot(x=seq(min(habitat[]),max(habitat[]),0.1),a1=a1,beta=logit(0.1))

ggplot(d,aes(x=x,y=y,col=State)) + geom_line() + geom_point() +  labs(x="Ocean Depth (km)",y="Probability of State") + theme_bw() 
...
```{r}
if(newModel){
    mxy<-lapply(1:3,function(x){
      traj(gamma=gamma,theta=theta,a1=a1,habitat=habitat,beta=beta)
    }) 
...
ggplot(mxy,aes(x=x,y=y)) + theme_bw() + coord_cartesian(xlim=bb[1,],ylim=bb[2,]) +  geom_raster(data=habitatdf,aes(x=x,y=y,fill=layer)) + scale_fill_continuous("Ocean Depth",low='grey90',high='black')    + geom_path(aes(col=State,group=Animal),size=1) + facet_wrap(~Animal)
...
#steps 
steps<-as.numeric(table(mxy$Animal)[1])
...
if(newModel){
  jags<-jags.model(file = "Bayesian/Multi_RW.jags",data=data,n.chains=2)
  }

system.time(update(jags,6000))
p<-coda.samples(model=jags,n.iter=2000,thin=5,variable.names=c("theta","gamma","alpha","beta","phi"))
```

##Chains
```{r,fig.height=10}
#bind chains
pc<-melt(list(as.matrix(p[[1]]),as.matrix(p[[2]])))
colnames(pc)<-c("Draw","par","value","chain")
...
#single index
splitpc[c("gamma","theta")]<-lapply(splitpc[c("gamma","theta")],function(x){
...
#plot all but d
ggplot(pc[!pc$parameter %in% c("phi"),],aes(x=Draw,y=value)) + facet_wrap(~par,scales="free") + geom_line(aes(col=as.factor(chain))) + labs(col="Chain")
```

```{r}
#true values
tv<-melt(list('alpha'=inv.logit(a1),'theta[2]'=theta[2],'theta[1]'=theta[1],'gamma[1]'=gamma[1],'gamma[2]'=gamma[2],'beta'=inv.logit(beta)))
...
#add priors
#alpha
a1prior<-data.frame(Draw=1:todraw,par='alpha',value=inv.logit(rnorm(todraw,0,1.6)),parameter='alpha',Estimate="Prior")

beta1prior<-data.frame(Draw=1:todraw,par='beta',value=inv.logit(rnorm(todraw,0,1.6)),parameter='beta',Estimate="Prior")
...
prs<-rbind_all(list(a1prior,beta1prior,gamma1prior,gamma2prior))
...
```{r}
alphap<-pc %>% filter(parameter=='alpha') %>% .$value

betap<-pc %>% filter(par=='beta') %>% .$value
 
postplot<-list()

for(j in 1:length(alphap)){
  postplot[[j]]<-data.frame(dplot(x=mxy$ocean,a1=logit(alphap[j]),beta=logit(betap[j])),Iteration=j)
  }

postplot<-rbind_all(postplot)

dsum<-postplot %>% group_by(x,State) %>% summarize(y_mean=mean(y),lower=quantile(y,0.05),upper=quantile(y,0.95))

ggplot(dsum,aes(x=x,y=y_mean,ymin=upper,ymax=lower,fill=State)) + geom_ribbon() + theme_bw() + labs(col="Transition",x="Ocean Depth (km)",y="Probability",fill="Transition")
```
