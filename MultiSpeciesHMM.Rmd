...
#paramters to track
pt<-c("theta","gamma","phi","lalpha_mu","lalpha_tau","lbeta_mu","lbeta_tau")
...
#Double index
#splitpc[c("alpha","beta")]<-lapply(splitpc[c("alpha","beta")],function(x){
#As matrices
 #   sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+),(\\d+)]"))[,3:4]
  #  colnames(sv)<-c("Animal","Behavior")
   # pc<-data.frame(x,sv)
#})
...
#plot all but d
ggplot(pc[!pc$parameter %in% c("phi","alpha","beta"),],aes(x=Draw,y=value)) + facet_wrap(~par,scales="free",ncol=2) + geom_line(aes(col=as.factor(chain))) + labs(col="Chain")

ggplot(pc[pc$parameter %in% c("alpha","beta"),],aes(x=Draw,y=value)) + facet_grid(parameter~Animal,scales="free") + geom_line(aes(col=as.factor(chain))) + labs(col="Chain")
...
ggplot() + geom_rect(data=state_shade,aes(xmin=xmin,xmax=xmax,fill=Behavior), ymin=-Inf,ymax=Inf) + facet_wrap(~Animal,ncol=1) + theme_bw() + geom_line(data=mxy,aes(x=Step,y=phi)) + ylim(0,1) + labs(y="Estimated Probability of Feeding")
```
