...
ggplot(data=spp_b,aes(x=x,y=y,col=Behavior)) + geom_point() + theme_bw() + geom_path(data=dxy,aes(x=x,y=y),col='red')

#merge with input
dxy$step<-1:nrow(dxy)
dxy<-merge(dxy,tb,by="step")

ggplot()+geom_path(data=dxy, aes(x=x, y=y,col=Probability),size=2) + scale_color_continuous(low='blue',high='red') + labs(col="Probability of Feeding")
```

##Autocorrelation in behavior

```{r}
ggplot(data=dxy,aes(x=as.numeric(step),y=Probability,col=ocean)) + geom_line(aes(group=1),size=2.5) + labs("Probability of Feeding") + theme_bw() + scale_color_continuous(low='light blue',high='black') + labs(x="Time Step",y="Liklihood of Feeding")
```
