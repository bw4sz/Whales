...
traj<-function(gamma,theta,rho=0){
  ...
  rw<-function(mu,Sigma){
    mvrnorm(n=1,mu,Sigma=Sigma)
  }

  #Draw random walk locations
  for (x in 2:steps){
    #Displacement
    d[x,]<-gamma*T %*% d[x-1,] ## matrix multiplication

    #next position
    xy[x,]<-rw(mu=(xy[x-1,] + d[x,]),Sigma)
  }
...
jags<-jags.model(file = "Bayesian/RW.jags",data=data,n.chains=2,n.adapt=2000 )
system.time(update(jags,20000))
p<-coda.samples(model=jags,n.iter=3000,n.thin=5,variable.names=c("Sigma","theta","gamma"))
```
...
```{r}
#true values
tv<-melt(list(theta=theta,gamma=gamma,Sigma=0.1))
