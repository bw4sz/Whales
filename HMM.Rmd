...
crop_habitat<-crop(habitat,bb*10)
...
p<-coda.samples(model=jags,n.iter=100,thin=10,variable.names=c("theta","gamma","alpha","beta"))
```
...
```{r,eval=T}
pr<-coda.samples(model=jags,n.iter=100,n.thin=5,variable.names=c("d"))
ps<-rbind(pr[[1]],pr[[2]])
ps<-melt(ps)
...
behav<-coda.samples(model=jags,n.iter=100,n.thin=5,variable.names=c("phi"))
