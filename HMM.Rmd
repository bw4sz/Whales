...
$$\gamma = 1 = Strong Movement Persistance$$
$$\theta = 0 = No preference in turns$$
$$ \rho = 0 = no correlation between heading northwest or southeast$$
$$ \sigma_lat = 0.1$$
$$ \sigma_lon = 0.1$$

These are mimicing values Jonsen 2005 fit for foraging seals.

```{r}
gamma=1
theta=0
dxy<-traj(gamma=1,theta=0)
...
ggplot(mxy,aes(x=x,y=y)) + geom_point(size=5,col='black') + geom_path(aes(col=as.factor(L1)),size=2) + theme_bw() + scale_color_discrete(guide=F) + ggtitle("Multiple trajectories")
```
...
##Prediction
```{r,eval=F}
pr<-coda.samples(model=jags,n.iter=5000,n.thin=1,variable.names=c("ynew"))
ps<-posummary(pr)
ps<-melt(ps)
