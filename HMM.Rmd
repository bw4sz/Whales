...
#Simulation
Generate travel paths for whales based on parameter values.

##Behavioral States
$$ \alpha_{1,1} = \text(Probability of remaining feeding when feeding) = 0.7$$
$\alpha_{2,1} = \text(Probability of switching from feeding to traveling) = 0.4$
...
## Foraging
$$\gamma_1 = 0.2 = \text{Weak Movement Persistence}$$
$$\theta_1 = pi = \text{Many reversals in turns}$$

## Traveling
$$\gamma_2 = 0.8 = \text{Strong Movement Persistence}$$
...
p<-coda.samples(model=jags,n.iter=2000,n.thin=20,variable.names=c("Sigma","theta","gamma","alpha"))
```
...
```{r}
#true values
tv<-melt(list('alpha[1]'=a1[1],'alpha[2]'=a1[2],'theta[2]'=theta[2],'theta[1]'=theta[1],'gamma[1]'=gamma[1],'gamma[2]'=gamma[2],'Sigma[1,1]'=0.025,'Sigma[1,2]'=0,'Sigma[2,1]'=0,'Sigma[2,2]'=0.025))
...
```{r,eval=T}
pr<-coda.samples(model=jags,n.iter=1000,n.thin=4,variable.names=c("d"))
ps<-rbind(pr[[1]],pr[[2]])
ps<-melt(ps)
