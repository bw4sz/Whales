...
####Process Model
  ...
  for (x in 2:(steps-1)){
    ...
    phi[x,2] <- inv.logit(1-a1[state[x-1]]+beta[state[x-1]]*ocean[x])
...
#Create environment
m<-raster(mountains(7,0.5))
r<-(m-max(m[]))*-200/1000
ras2<-raster(xmn=-50,xmx=50,ymn=-50,ymx=50,resolution=0.5,crs=NA)
r2<-setExtent(r,ras2)
...
ggplot(dxy,aes(x=x,y=y)) + geom_raster(data=habitatdf,aes(x=x,y=y,fill=layer)) + geom_point(size=2,col='black') + geom_path(aes(col=State,group=1),size=1.5)  + theme_bw() + scale_fill_continuous("Ocean Depth",low='black',high='grey90') + coord_cartesian(xlim=bb[1,],ylim=bb[2,])
...
ggplot(mxy,aes(x=x,y=y)) + theme_bw() + coord_cartesian(xlim=bb[1,],ylim=bb[2,]) +  geom_raster(data=habitatdf,aes(x=x,y=y,fill=layer)) + scale_fill_continuous("Ocean Depth",low='black',high='grey90')    + geom_path(aes(col=State,group=L1),size=1) 

ggplot(mxy,aes(x=ocean,y=as.numeric(State)-1)) + geom_point() + stat_smooth(method="glm",method.args = list(family="binomial"),aes(group=L1),alpha=0.1) + scale_y_continuous(breaks=c(0,1),labels=c("Traveling","Feeding")) + ylab("Probability") + xlab("Ocean Depth") + theme_bw()
...
p<-coda.samples(model=jags,n.iter=10000,thin=10,variable.names=c("theta","gamma","alpha","beta"))
```
...
```{r}
#true values
tv<-melt(list('alpha[1]'=a1[1],'alpha[2]'=a1[2],'theta[2]'=theta[2],'theta[1]'=theta[1],'gamma[1]'=gamma[1],'gamma[2]'=gamma[2],'beta[1]'=beta[1],'beta[2]'=beta[2]))
