...
cat("
    ...
    for(i in 1:ind){
      for(g in 1:tracks[i]){
        ...
        logit(phi[i,g,t,1]) <- lalpha[i,state[i,g,t-1]] + lbeta[i,g,state[i,g,t-1]] * ocean[i,g,t]
    ...
    logit(phi[i,g,steps[i,g],1]) <- lalpha[i,g,state[i,g,steps[i,g]-1]] + lbeta[i,g,state[i,g,steps[i,g]-1]] * ocean[i,g,steps[i,g]]
