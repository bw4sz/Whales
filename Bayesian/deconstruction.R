...
cat("
    ...
    Tdx[t] <- T[,1] * d[t,1] + T[,2] * dt[t,2]	## matrix multiplication
    x.mn[t] <- x[t] + gamma * Tdx[t]	## estimation next location (no error)
    ...
    gamma ~ dbeta(1,1)	## prior for gamma (autocorrelation parameter) 
