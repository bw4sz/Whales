...
```{r}
names_of_vars<-c("ocean","coast","SST","ChloroA","WaveH","NPP","Ice")

posteriors<-list()
for (x in 1:length(names_of_vars)){
  posteriors[[x]]<-runModel(names_of_vars[[x]],mxy)
}
...
plotall<-list()
  ...
  for(x in 1:length(posteriors)){
    plotall[[x]]<-posteriors[[x]] %>% filter(parameter %in% c('beta_mu','alpha_mu')) %>% dcast(.,Draw+chain~par) %>% group_by(Draw,chain) %>% do(dplot(a1=c(.$'alpha_mu[1]',.$'alpha_mu[2]'),beta=c(.$'beta_mu[1]',.$'beta_mu[2]'),x=unique(mxy[[names(posteriors)[x]]]))) %>% group_by(x=x,Begin,State) %>% summarize(mean=mean(y),lower=quantile(y,0.05),upper=quantile(y,0.95))  
  }
...
monthall<-list()
  ...
  #get the posterior estimates of the env function
  postplot_alpha<-posteriors[[x]] %>% filter(parameter %in% c('alpha_mu')) %>% do(dcast(.,Draw+chain~par))
  postplot_beta<-posteriors[[x]] %>% filter(parameter %in% c('beta')) %>% dcast(.,Month+Draw+chain~parameter+Behavior)
  postall<-merge(postplot_alpha,postplot_beta,by=c("Draw","chain"))

  monthall[[x]]<-postall %>% group_by(Month,Draw,chain) %>% do(dplot(a1=c(.$'alpha_mu[1]',.$'alpha_mu[2]'),beta=c(.$'beta_1',.$'beta_2'),x=unique(mxy[[names(posteriors)[x]]]))) %>% group_by(x=x,Month,Begin,State) %>% summarize(mean=mean(y),lower=quantile(y,0.05),upper=quantile(y,0.95))
   }
...
ggplot(monthall) + geom_ribbon(aes(x=x,y=mean,ymin=upper,ymax=lower,fill=Month),alpha=0.5) + theme_bw() + labs(col="Transition",x="Ocean Depth (km)",y="Probability",fill="Transition") + facet_wrap(~L1,scales="free")

ggplot(monthall) + geom_line(aes(x=x,y=mean,ymin=upper,ymax=lower,col=Month)) + theme_bw() + labs(col="Transition",x="Ocean Depth (km)",y="Probability",fill="Transition") + facet_wrap(~L1,scales="free")
